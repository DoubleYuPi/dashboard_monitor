...
DEFAULT_COLUMNS = [
    ...
    "tgl_pengumuman_nontender", "status_nontender"
]
PAGE_SIZE_OPTIONS = [10, 25, 50, 100]

st.set_page_config(page_title="Rincian Non-Tender", layout="wide")
...
class InaprocAPIClient:
    ...
    def get_nontender(self, kode_klpd: str, tahun: int):
        response = requests.get(
            f"{self.base_url}/legacy/tender/non-tender-pengumuman",
...
def fetch_data(kode_klpd, tahun, token):
    return InaprocAPIClient(token).get_nontender(kode_klpd, tahun)
...
def show_table(df, page_key):
    ...
    with col_status:
        if "status_nontender" in df_display.columns:
            status_list = ["Semua"] + sorted(df["status_nontender"].dropna().unique().tolist())
            selected_status = st.selectbox("Status NonTender", status_list, key=f"status_{page_key}")
        else:
    ...
    if selected_status != "Semua":
        df_display = df_display[df_display["status_nontender"] == selected_status]
    ...
    st.download_button(
        ...
        csv, f"nontender_{kode_klpd}_{tahun}.csv", "text/csv",
...
# --- Main Page ---
st.title("📋 Rincian Paket Non-Tender")
...
try:
    ...
    st.success(f"✅ {len(df)} paket non-tender ditemukan untuk KLPD {kode_klpd} tahun {tahun}")
    show_table(df, page_key="nontender")
