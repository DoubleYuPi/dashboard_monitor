...
# --- Config ---
DEFAULT_COLUMNS = [
    "nama_satker", "nama_paket", "spesifikasi_pekerjaan",
    "pagu", "jenis_pengadaan", "metode_pengadaan", "tanggal_buat_paket"
]
PAGE_SIZE_OPTIONS = [10, 25, 50, 100]

st.set_page_config(page_title="INAPROC Monitor", layout="wide")
...
class InaprocAPIClient:
    ...
    def get_penyedia(self, kode_klpd, tahun):
        return self._fetch_json("/legacy/rup/paket-penyedia-terumumkan", {"kode_klpd": kode_klpd, "tahun": tahun})

    def get_swakelola(self, kode_klpd, tahun):
        return self._fetch_json("/legacy/rup/paket-swakelola-terumumkan", {"kode_klpd": kode_klpd, "tahun": tahun})
...
def fetch_penyedia(kode_klpd, tahun, token):
    return InaprocAPIClient(token).get_penyedia(kode_klpd, tahun)

@st.cache_data(ttl=60)
def fetch_swakelola(kode_klpd, tahun, token):
    return InaprocAPIClient(token).get_swakelola(kode_klpd, tahun)
...
st.sidebar.markdown("---")
st.sidebar.header("📁 Rincian Paket")
menu = st.sidebar.radio(
    label="Pilih Jenis Paket",
    options=["📦 Penyedia", "🤝 Swakelola"],
...
def show_table(df, page_key):
    ...
    col_satker, col_size, col_metode, col_jenis, _ = st.columns([2, 1, 1, 1, 1])
    ...
    label_csv = "penyedia" if page_key == "penyedia" else "swakelola"
    st.download_button(
        "⬇️ Download CSV (semua kolom)",
        csv, f"rup_{label_csv}_{kode_klpd}_{tahun}.csv", "text/csv",
...
# --- Page: Penyedia ---
if menu == "📦 Penyedia":
    st.title("📦 Rincian Paket Penyedia")
    st.caption(f"KLPD: **{kode_klpd}** | Tahun: **{tahun}**")

    if st.button("← Kembali ke Dashboard"):  
        st.switch_page("home.py")
    try:
        raw = fetch_penyedia(kode_klpd, tahun, JWT_TOKEN)
        df = parse_response(raw)
        st.success(f"✅ {len(df)} paket penyedia ditemukan")
        show_table(df, page_key="penyedia")
...
# --- Page: Swakelola ---
    ...
    st.title("🤝 Rincian Paket Swakelola")
    st.caption(f"KLPD: **{kode_klpd}** | Tahun: **{tahun}**")

    if st.button("← Kembali ke Dashboard", key="back_swakelola"):  # 👈 Add this
        st.switch_page("home.py")
    try:
        raw = fetch_swakelola(kode_klpd, tahun, JWT_TOKEN)
        df = parse_response(raw)
        st.success(f"✅ {len(df)} paket swakelola ditemukan")
        show_table(df, page_key="swakelola")
    except requests.exceptions.HTTPError as e:
        st.error(f"❌ API Error {e.response.status_code}: {e.response.text}")
    except Exception as e:
        st.error(f"❌ Error: {e}")
